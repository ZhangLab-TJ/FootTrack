...
from foottrack.parsers import add_scorebigwig_arguments
...
from foottrack.utils.signals import *
from foottrack.utils.logger import foottrackLogger

#--------------------------------------------------------------------------------#
def calculate_scores(regions, args):

	logger = foottrackLogger("", args.verbosity, args.log_q)

	pybw_signal = pyBigWig.open(args.signal) 	#conversion rate signal
	...
	for i, region in enumerate(regions):
		...
		if args.score == "sum":
			...
			scores = foottrack_footprint_array(signal, args.flank_min, args.flank_max, args.fp_min, args.fp_max)		#numpy array
...
def run_scorebigwig(args):
	...
	logger = foottrackLogger("ScoreBigwig", args.verbosity)
	logger.begin()
	...
	logger.info("- Opening input conversion bigwig")
