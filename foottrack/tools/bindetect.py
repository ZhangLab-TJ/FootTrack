...
from foottrack.parsers import add_bindetect_arguments
from foottrack.tools.bindetect_functions import *
from foottrack.utils.utilities import *
from foottrack.utils.regions import *
from foottrack.utils.motifs import *
...
def run_bindetect(args):
	...
	logger = foottrackLogger("BINDetect", args.verbosity)
	logger.begin()
	...
	for i, peak in enumerate(peaks):
		if len(peak) != n_cols:
			logger.error("The lines in --peaks have a varying number of columns. Line 1 has {0} columns, but line {1} has {2} columns! Please adjust the format of this file to run FootTrack BINDetect.".format(n_cols, i+1, len(peak)))
			sys.exit(1)
	...
	# Collect sampled background values and create a filtered copy for normalization
	background_no_zero = {}
	for bigwig in args.cond_names:
		signal_array = np.array(background["signal"][bigwig])
		# Remove 0s for normalization
		background_no_zero[bigwig] = signal_array[np.logical_not(np.isclose(signal_array, 0.0))]
		# Keep original background unchanged
		background["signal"][bigwig] = signal_array

	# Check how many values were fetched from background
	n_bg_values = len(background_no_zero[args.cond_names[0]])
	logger.debug("Collected {0} values from background (non-zero)".format(n_bg_values))
	if n_bg_values < 1000:
	...
	# Plot score distribution (non-zero)
	fig = plot_score_distribution([background_no_zero[bigwig] for bigwig in args.cond_names], labels=args.cond_names, title="Raw scores per condition (Non-Zero)")
	figure_pdf.savefig(fig, bbox_inches='tight')
	plt.close()

	# Normalize arrays (on non-zero data)
	args.norm_objects = {}
	if args.norm_off == True or len(args.cond_names) == 1: #if norm_off or length of cond is 1 - create constant normalization
		...
		list_of_vals = [background_no_zero[bigwig] for bigwig in args.cond_names]
		if args.debug: 
		...
		# Normalize background (non-zero) and visualize score distribution
		for bigwig in args.cond_names:
			original = background_no_zero[bigwig]

			# Check for nan
			logger.debug("Background nans (non-zero, {0}): {1}".format(bigwig, sum(np.isnan(original))))
			...
			# Add the removed 0 values back to the normalized data
			original_with_zeros = np.zeros_like(background["signal"][bigwig])
			original_with_zeros[np.logical_not(np.isclose(background["signal"][bigwig], 0.0))] = normalized

			# Save the final normalized data with zeros added back
			background["signal"][bigwig] = original_with_zeros

			logger.debug("Background nans after normalization with zeros added back ({0}): {1}".format(bigwig, sum(np.isnan(background["signal"][bigwig]))))
		
		# Plot normalized score distribution without zero values
		filtered_signals = [
			background["signal"][bigwig][np.logical_not(np.isclose(background["signal"][bigwig], 0.0))]
			for bigwig in args.cond_names
		]

		fig = plot_score_distribution(filtered_signals, labels=args.cond_names, title="Normalized scores per condition (Non-Zero)")
		figure_pdf.savefig(fig, bbox_inches='tight')
		plt.close()
	...
	# max_len = max(len(background["signal"][bigwig]) for bigwig in args.cond_names)
	# bg_values = np.array([
	# 	np.pad(np.array(background["signal"][bigwig]), (0, max_len - len(background["signal"][bigwig])), 'constant', constant_values=(0,))
	# 	for bigwig in args.cond_names
	# ]).flatten()
	...
	x_max = np.nanpercentile(bg_values, [99]) 
	...
	lowest_bic = np.inf
	if log_vals.size == 0:
		print("Error: No data available for Gaussian Mixture Model.")
		return  # 直接返回或者处理错误
	...
	for (cond1, cond2) in comparisons:
		...
		pval_min = np.nanpercentile(pvalues[pvalues > 0], 5)	#5% smallest pvalues
		change_min, change_max = np.nanpercentile(changes, [5, 95])	#5% smallest and largest changes
