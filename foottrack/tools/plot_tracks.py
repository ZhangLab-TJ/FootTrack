...
from foottrack.parsers import add_tracks_arguments
from foottrack.utils.utilities import *

#--------------------------------------------------------------------------------------------------------#
def svist4get_defaults():
	""" Config parameters used to visualize FootTrack tracks """
...
def run_tracks(args):
	...
	#Setup logger
	logger = foottrackLogger("PlotTracks", args.verbosity)
	logger.begin()
	...
	with open(args.regions) as f:
		for line in f:
			...
			pa.config['bedgraph_upper_limit'] = "auto"
			pa.config['bedgraph_lower_limit'] = "auto"
