...
from foottrack.parsers import add_motifclust_arguments
from foottrack.utils.utilities import check_required, check_files, make_directory
...
def run_motifclust(args):
	...
	logger = foottrackLogger("ClusterMotifs", args.verbosity)
	logger.begin()
	...
	try:
		...
		if e.name == "collections" and (version.parse(python_version) >= version.parse("3.10.0")): #collections error from norns=0.1.5 and from other packages on python=3.10
			logger.error("Due to package dependency errors, 'FootTrack ClusterMotifs' is not available for python>=3.10. Current python version is '{0}'. Please downgrade python in order to use this tool.".format(python_version))
			sys.exit(1)
