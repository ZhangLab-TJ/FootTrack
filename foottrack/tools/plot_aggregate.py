...
#Internal classes
from foottrack.parsers import add_aggregate_arguments
from foottrack.utils.utilities import check_required, check_files
from foottrack.utils.regions import OneRegion, RegionList
from foottrack.utils.logger import foottrackLogger, add_logger_args
...
def clean_data(agg1, agg2):
    # 检查 NaN 和 inf 值
    mask = ~np.isnan(agg1) & ~np.isnan(agg2) & np.isfinite(agg1) & np.isfinite(agg2)
    return agg1[mask], agg2[mask]
...
def run_aggregate(args):
	...
	logger = foottrackLogger("PlotAggregate", args.verbosity)
	logger.begin()
	...
	for row, signal_name in enumerate(signal_names):	
		for col, region_name in enumerate(region_names):
			...
			if signalmat.shape[0] == 0: #no regions
				...
				aggregate = np.nan(args.width)
			else:	

				#Exclude outlier rows 
				max_values = np.nanmax(signalmat, axis=1)
				upper_limit = np.nanpercentile(max_values, [100*args.remove_outliers])[0]	#remove-outliers is a fraction
	...
	for row, signal_name in enumerate(signal_names):	
		for col, region_name in enumerate(region_names):
			...
			for fp_flank in range(int(motif_width/2), min([25, args.flank])):	#motif width for this bed
				...
				baseline = np.nanmean(agg[baseline_indices])

				#Footprint level
				middle_indices = list(range(args.flank-fp_flank, args.flank+fp_flank))
				middle = np.nanmean(agg[middle_indices]) 	#within the motif
	...
	for ax1, ax2 in combis:
		...
		cleaned_agg1, cleaned_agg2 = clean_data(agg1, agg2)
		pearsonr, pval = scipy.stats.pearsonr(cleaned_agg1, cleaned_agg2)

		diff = np.nansum(np.abs(agg1 - agg2))	#Sum of difference between agg1 and agg2
	...
	#Comparable rows (rowcompare = same across all)
		...
		for col in range(n_cols):
			...
			ymin, ymax = np.nanmin(lims), np.nanmax(lims)
