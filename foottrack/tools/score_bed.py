...
#Utils from FootTrack
from foottrack.parsers import add_scorebed_arguments
...
def get_score_func(args):
	...
	if args.position == "start":
		...
		if args.math == "min":
			func = lambda signal: np.nanmin(signal)
		elif args.math == "max":	
			func = lambda signal: np.nanmax(signal)
		elif args.math == "mean":
			func = lambda signal: np.nanmean(signal)
		elif args.math == "sum":
			func = lambda signal: np.nansum(signal)
...
def run_scorebed(args):
	...
	#Start logger
	logger = foottrackLogger("ScoreBed", args.verbosity)
	logger.begin()
