...
def quantile_normalization(list_of_arrays, names, pdfpages=None, logger=foottrackLogger()): #lists paired values to normalize
...
def scan_and_score(regions, motifs_obj, args, log_q, qs):
	...
	logger = foottrackLogger("", args.verbosity, log_q)	#sending all logger calls to log_q
...
def process_tfbs(TF_name, args, log2fc_params): 	#per tf
	...
	logger = foottrackLogger("", args.verbosity, args.log_q) 	#sending all logger calls to log_q
