...
from foottrack.parsers import add_heatmap_arguments
...
def run_heatmap(args):
	...
	#Start logger
	logger = foottrackLogger("PlotHeatmap", args.verbosity)
	logger.begin()
	...
	for col in range(len(args.TFBS)):
		...
		for row in heatmap_info[col]:
			...
			# 	heatmap_info[col][row]["aggregate"] = np.nanmean(heatmap_info[col][row]["signal_mat"], axis=0) 

			if len(heatmap_info[col][row]["regions"]) > 0:
				signal_mats = []
				for region in heatmap_info[col][row]["regions"]:
					signal = region.get_signal(pybw)
					signal = np.array(signal)
					signal[signal == None] = np.nan  # 将没有数值的位点设置为NA (np.nan)
					signal_mats.append(signal)
					# print(signal_mats)
				heatmap_info[col][row]["signal_mat"] = np.array(signal_mats)
				heatmap_info[col][row]["aggregate"] = np.nanmean(heatmap_info[col][row]["signal_mat"], axis=0)  # 忽略NA的值
	...
	if args.share_colorbar == True:
		...
		if len(mats) > 0:
			...
			vmin, vmax = np.nanpercentile(joined, [1, 99])
	...
	for i, name in enumerate(args.show_columns):
		...
		vmin, vmax = np.nanpercentile(glob_values, [1, 99])
		for row in range(len(args.TFBS[0])):
	...
	for col, bigwig in enumerate(args.signals):
		...
		for row in heatmap_info[col]:
			if heatmap_info[col][row]["signal_mat"] is not None:
				...
				#Heatmap
				# lim = np.nanmax([np.abs(heatmap_info[col][row]["vmin"]),np.abs(heatmap_info[col][row]["vmax"])])
				...
				# cmap = plt.get_cmap(args.cmap)  # Get the colormap
				# cmap = cmap.copy()  # Make a copy of the colormap
				# cmap.set_bad(color='red')  # Set color for NaN values

				# Aggregate
				axdict[col]["aggregate"].plot(xvals, heatmap_info[col][row]["aggregate"], color=colors[row], linewidth=2, label=args.TFBS_labels[col][row])

				vmin = np.nanmin(heatmap_info[col][row]["signal_mat"])
				vmax = np.nanmax(heatmap_info[col][row]["signal_mat"])
				heatmap = axdict[col][row].imshow(heatmap_info[col][row]["signal_mat"], aspect="auto", cmap=args.cmap, norm=mpl.colors.Normalize(vmin=vmin, vmax=vmax))
	...
	for i, col in enumerate(range(no_signals, no_signals + no_columns)):
		...
		for row in heatmap_info[col]:
			...
			lim = np.nanmax([abs(vmin), abs(vmax)])
