...
def add_subsample_arguments(parser):
	...
	description = "Subsample a .bw-file into random subsets (of percentage size --start to --end with step --step).\n"
	...
	args.add_argument('--bw', metavar="", help="Path to .bw-file")
	...
	args.add_argument('--outdir', metavar="", help="Output directory (default: subsamplebw_output)", default="subsamplebw_output")
	args.add_argument('--prefix', metavar="", help="Prefix for output files (default: prefix of .bw)")
	args.add_argument('--force', action="store_true", help="Force creation of subsampled .bw-files (default: only create if not existing)")
...
def add_filterfragments_arguments(parser):
	...
	parser.formatter_class = lambda prog: argparse.RawDescriptionHelpFormatter(prog, max_help_position=40, width=90)
	description = "FilterFragments can filter out fragments from a .bw-file of paired-end reads based on the overlap with regions in the given .bed-file."
	#description += "Usage: "
	...
	IO = parser.add_argument_group('Input / output arguments')
	IO.add_argument('--bw', metavar="", help=".bw-file to filter")
	...
	IO.add_argument('--output', metavar="", help="Path to the filtered .bw-file (default: <prefix of --bw>_filtered.bw)")
	IO.add_argument('--threads', metavar="", type=int, help="Number of threads used for decompressing/compressing bw (default: 10)", default=10)
