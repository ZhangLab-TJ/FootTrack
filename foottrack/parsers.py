...
def add_biascorrect_arguments(parser):
	...
	description = "BiasCorrect corrects the conversion-signal from cFOOT-seq with regard to the underlying sequence preference of deaminase.\n\n"
	...
	optargs.add_argument('-p', '--peaks', metavar="<bed>", help="A .bed-file containing analysis peak regions (default: the full range of .bw-file)")
	optargs.add_argument('--mode', help="Specify background mode: 'global' or 'local' (default: global)", choices=["global", "local"], default="global", type=str)
	...
	optargs.add_argument('--track-off', metavar="<track>", help="Switch off writing of individual .bigwig-tracks (bias/corrected/expected) ", nargs="*", default=[])
	...
	optargs.add_argument('--k_flank', metavar="<int>", help="Flank +/- of conversion to estimate bias from (default: 10)", type=int, default=10)
	...
	optargs.add_argument('--standard', metavar="<float>", help="Standard for background estimation (useful when --mode is 'global') (default: mean of conversion rate)", type=float)
	optargs.add_argument('--window', metavar="<int>", help="Window size for calculating expected signal (useful when --mode is 'local') (default: 100)", type=int, default=100)
...
#--------------------------------------------------------------------------------------------------------#
def add_depth_biascorrect_arguments(parser):

	parser.formatter_class = lambda prog: argparse.RawDescriptionHelpFormatter(prog, max_help_position=35, width=90)

	description = "DepthBiascorrect corrects the data from cFOOT-seq.\n\n"
	description += "Usage:\nFootTrack DepthBiascorrect --input <input.txt> --genome <genome.fa> --peaks <peaks.bed> --bias <bias.pickle>\n\n"
	description += "Output files:\n"
	description += "- <prefix>.txt"
	parser.description = format_help_description("DepthBiascorrect", description)

	parser._action_groups.pop()	#pop -h

	#Required arguments
	reqargs = parser.add_argument_group('Required arguments')
	reqargs.add_argument('-i', '--input', metavar="<txt>", help="A .txt-file containing chr start end strand conversion_ratio  N_convert N_total")
	reqargs.add_argument('-g', '--genome', metavar="<fasta>", help="A .fasta-file containing whole genomic sequence")
	reqargs.add_argument('-p', '--peaks', metavar="<bed>", help="A .bed-file containing analysis peak regions")
	reqargs.add_argument('-b', '--bias', metavar="<obj>", help="Path to a pre-calculated enzBias.pkl-object, as output from a previous DepthBiascorrect run (default: None). Can be used to bypass the internal bias estimation.")

	#Optional arguments
	optargs = parser.add_argument_group('Optional arguments')
	optargs = parser.add_argument_group('Optional arguments')
	optargs.add_argument('--extend', metavar="<int>", type=int, help="Extend output regions with basepairs upstream/downstream (default: 100)", default=100)
	optargs.add_argument('--split-strands', help="Write out tracks per strand (default: False)", action="store_true", default=False)
	optargs.add_argument('--mode', help="Specify background mode: 'global' or 'local' (default: global)", choices=["global", "local"], default="global", type=str)
	optargs.add_argument('--k_flank', metavar="<int>", help="Flank +/- of conversion to estimate bias from (default: 10)", type=int, default=10)
	optargs.add_argument('--standard', metavar="<float>", help="Standard for background estimation (useful when --mode is 'global') (default: mean of conversion rate)", type=float)
	optargs.add_argument('--window', metavar="<int>", help="Window size for calculating expected signal (useful when --mode is 'local') (default: 100)", type=int, default=100)
...
def add_scorebigwig_arguments(parser):
	...
	required.add_argument('-s', '--signal', metavar="<bigwig>", help="A .bw file of cFOOT-seq conversion signal")
...
def add_tfbscan_arguments(parser):
	...
	description += "Usage:\nFootTrack TFBScan --motifs <motifs.txt> --fasta <genome.fa> \n\n"
...
def add_aggregate_arguments(parser):
	...
	IO.add_argument('--output', metavar="", help="Path to output plot (default: FootTrack_aggregate.pdf)", default="FootTrack_aggregate.pdf")
...
def add_plotchanges_arguments(parser):
	...
	parser.formatter_class = lambda prog: argparse.RawDescriptionHelpFormatter(prog, max_help_position=35, width=90)
	description = "PlotChanges is a utility to plot the changes in TF binding across multiple conditions as predicted by FootTrack BINDetect.\n\n"
	description += "Example usage:\n$ echo CTCF GATA > TFS.txt\n$ FootTrack PlotChanges --bindetect <bindetect_results.txt> --TFS TFS.txt\n\n"
...
def add_heatmap_arguments(parser):
	...
	IO.add_argument('--output',  metavar="", help="Output filename (default: FootTrack_heatmap.pdf)", default="FootTrack_heatmap.pdf")
	...
	PLOT.add_argument('--title', metavar="", default="FootTrack heatmap")
...
def add_tracks_arguments(parser):
	...
	parser.formatter_class = lambda prog: argparse.RawDescriptionHelpFormatter(prog, max_help_position=40, width=90)
	description = "Plot genomic tracks (such as conversion or footprint scores) in specific genomic regions.\n"
...
def add_maxpos_arguments(parser):
	...
	description += "Usage:\nFootTrack MaxPos --bed <regions.bed> --bigwig <signal.bw>\n\n"
...
def add_motifclust_arguments(parser):
	...
	description += "Usage:\nFootTrack ClusterMotifs --motifs <motifs.jaspar>\n\n"
...
def add_downloaddata_arguments(parser):
	...
	parser.formatter_class = lambda prog: argparse.RawDescriptionHelpFormatter(prog, max_help_position=40, width=90)
	description = "Download test data for the FootTrack commandline examples"
	...
	arguments.add_argument('--bucket', metavar="", help="Name of bucket to download (default: data-foottrack-2020)", default="data-foottrack-2020")
