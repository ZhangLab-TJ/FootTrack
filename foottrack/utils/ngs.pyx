...
#--------------------------------------------------------------------------------------------------#
def index_bw(bw_obj, logger=None):
	""" 
	Function to index a bw file if the index does not exist

	Parameters
	-----------
	bw_obj : pysam.AlignmentFile object
		bw_obj with/without associated
	Returns
	-----------
	bw_obj : pysam.Alignment object
		bw_obj with index
	"""

	filename = bw_obj.filename.decode('utf-8')
	...
	try:
		...
		bw_obj = pysam.AlignmentFile(filename)	#open bw again to include index 
...
class OneRead():
	...
	def __init__(self, read=None):
		...
		#New info on read
		self.conversion = None		#no conversion position yet
		self.kmer = []			#kmer in integer nucleotide code around conversion
	...
	def get_conversion(self, read_shift=(4,-5)):
		""" Finds the conversion position from read taking direction & read shift into account 
		...
		if self.is_reverse == True:
			self.conversion = positions[1] + 1 + neg_shift  	#Position to the right of read start denotes conversion
			
		#+ strand
		else:				
			self.conversion = positions[0] + 1 + pos_shift		#.bw start is 0 based, so +1 converts to true genomic coordinates. 


	def get_kmer(self, genomic_sequence, k_flank):
		""" Extract kmer from genomic_sequence object around conversion """

		seq_start, seq_end = genomic_sequence.region.start, genomic_sequence.region.end		#excluding start, including end
		conversion = self.conversion

		if conversion > seq_start + k_flank + 1 and conversion <= seq_end - k_flank:

			#reverse read
			if self.is_reverse == True:
				i = seq_end - conversion  		#position in revcomp_num
				self.kmer = genomic_sequence.revcomp[i-k_flank:i+k_flank+1] 

			#forward read
			else:
				i = conversion - 1 - seq_start  	#-1 for 0-based python coordinates	
				self.kmer = genomic_sequence.sequence[i-k_flank:i+k_flank+1]

		else:
			self.kmer = np.array([4] * (k_flank*2 + 1))

	def shift_conversion(self, bp):
		""" Shift the position of conversion by bp 
...
class ReadList(list):
	...
	def from_bw(self, bw_obj, region):
		try:
			read_list = bw_obj.fetch(region.chrom, region.start, region.end)
			self = ReadList([OneRead(read) for read in read_list if read.is_unmapped == False and read.is_duplicate == False])

		except Exception as e:
			sys.exit("Error reading {0} from bw object".format(region))
