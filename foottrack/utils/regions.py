...
class OneRegion(list):
	...
	def check_boundary(self, boundaries_dict, action="cut", logger=foottrackLogger()):
	...
	def get_signal(self, pybw, numpy_bool = True, logger=foottrackLogger(), key=None):
...
class RegionList(list):
	...
	def from_bed(self, bedfile_f, logger=foottrackLogger()):
		...
		try:
			...
			logger.error("Could not read input file '{0}'. It looks like the file might be compressed. FootTrack only reads .bed-files in raw text format. Exception is: '{1}'".format(bedfile_f, e))
			sys.exit(1)
		except Exception as e:
