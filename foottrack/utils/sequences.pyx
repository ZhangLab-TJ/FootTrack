...
class NucleotideMatrix(SequenceMatrix):
	...
	def add_sequence(self, np.ndarray[np.int_t, ndim=1] sequence, double amount = 1.0):
		...
		for m in range(length):
			...
			if amount < 0:
				neg_counts[Sm,m] += amount
			else:
				counts[Sm,m] += amount	#addition of minus = subtraction
	...
	def prepare_mat(self):
		...
		self.bias_pwm = np.true_divide(self.counts, self.no_bias)
		self.bg_pwm = np.true_divide(self.bg_counts, self.no_bg)

		#Prepare PSSM for scoring
		self.pssm = np.log2(np.true_divide(self.bias_pwm + epsilon , self.bg_pwm + epsilon ))
...
class GenomicSequence:
	""" Used in BiasCorrect for fast scoring of PWM/DWM """
