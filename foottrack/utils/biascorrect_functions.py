...
def bias_correction(regions_lst, params, bias_obj, standard):
	...
	for region_obj in regions_lst:
		...
		out_signals[reg_key] = {"bias":{}, "corrected":{}, "expected":{}}
		...
		if params.split_strands == False:
			for track in out_signals[reg_key]:
				...
				print("aaaaaaaaaaaaaaaaaaaaaaaaaaa", combined_sum)
				if combined_sum is None:
						print("警告: 输入数据combined_sum为 None，使用 NaN 填充的默认值")
		#Send to queue
...
def plot_pssm(matrix, title):
	...
	ax.xaxis.set_minor_locator(ticker.FixedLocator(xtick_pos))			#locate minor ticks between major ones (conversions)
...
def plot_correction(pre_mat, post_mat, title):
	...
	plt.xlim([0, L-1])
	plt.xlabel('Position from conversion')
