...
cat $dir/sample.txt | while read i; do
	...
	FootTrack BiasCorrect -b $ori_bw -g ${genome} -p $atac --cores ${core} --k_flank 10 --score_mat PWM --mood ${mood} --window 100 --prefix ${i} --outdir ./ --extend 0 --bias-pkl ${bias} --track-off bias expected --verbosity 5 >> ${i}_BiasCorrect.log 2>&1;
